...
def get_num_layer_for_vit(var_name, num_max_layer):
    if var_name in ('backbone.cls_token', 'backbone.register_tokens',
                    'backbone.pos_embed'):
        return 0
...
class LayerDecayOptimizerConstructor_Adapter(DefaultOptimWrapperConstructor):
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        for name, param in module.named_parameters():
            ...
            if len(param.shape) == 1 or name.endswith('.bias') \
                    or name in ('pos_embed', 'cls_token', 'register_tokens'):
