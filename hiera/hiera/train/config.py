...
class PretrainMedical(torch.utils.data.Dataset):
    def __init__(self, json_file="", prefix_path=None, transform=None):
        if os.path.exists(json_file):
            self.data = json.load(open(json_file))["train"]
...
class Dataset:
    ...
    def __call__(self, train: bool, img_size: int) -> torch.utils.data.Dataset:
        ...
        if self.type == "imagefolder":
            ...
            dataset = PretrainMedical(json_file=self.path, prefix_path=self.prefix_path, transform=transform)
        else:
