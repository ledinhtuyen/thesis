...
def train(model_name: str, config_name: str, continue_pretrain: bool,train_args: dict = {}):
    torch.set_float32_matmul_precision('medium')

    args = getattr(config.TrainArgs, config_name)(model_name).mutate(train_args)

    if "mae" in config_name:
        model = getattr(hiera, f"mae_{model_name}")(pretrained=continue_pretrain)
...
def make_arg_parser():
    ...
    parser.add_argument("--continue_pretrain", action="store_true", help="Continue pretraining from a checkpoint.")
...
def main(args: argparse.Namespace):
    ...
    train(args.model, args.config, args.continue_pretrain, train_args)
